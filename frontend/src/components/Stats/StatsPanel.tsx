...
function StatCard({ icon, label, value, subtext, color = 'cyan' }: StatCardProps) {
  return (
    <div className="bg-bg-secondary/50 rounded-lg p-2 lg:p-3 border border-cyan/10">
      <div className="flex items-center gap-1.5 lg:gap-2 mb-0.5 lg:mb-1">
        <div className={`text-${color}`}>{icon}</div>
        <span className="text-[10px] lg:text-xs text-text-secondary uppercase tracking-wider">
          {label}
        </span>
      </div>
      <div className={`font-mono text-lg lg:text-xl font-bold text-${color}`}>
        {value}
      </div>
      {subtext && (
        <div className="text-[10px] lg:text-xs text-text-secondary mt-0.5 lg:mt-1">
          {subtext}
        </div>
      )}
    </div>
...
export function StatsPanel() {
  ...
  return (
    <div className="card card-glow p-3 lg:p-4">
      <h2 className="font-display text-xs lg:text-sm text-cyan tracking-wider mb-3 lg:mb-4">
        LIVE STATISTICS
      </h2>

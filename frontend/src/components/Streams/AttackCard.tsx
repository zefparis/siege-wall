...
export function AttackCard({ attack }: AttackCardProps) {
  ...
  return (
    <motion.div
      ...
      className="attack-item flash-red bg-bg-secondary/50 rounded px-2 lg:px-3 py-1.5 lg:py-2 border-l-2"
    ...
    >
      <div className="flex items-center justify-between gap-1 lg:gap-2">
        ...
        <div className="flex items-center gap-1 lg:gap-2 min-w-0 flex-1">
          <span className="text-xs lg:text-sm">{categoryIcon}</span>
          <span className="font-mono text-[10px] lg:text-xs text-white truncate">
            {attack.attacker_name}
          </span>
          <span className="text-text-secondary text-[10px] lg:text-xs hidden sm:inline">
        ...
        <div className="flex items-center gap-1 lg:gap-2 flex-shrink-0">
          <span 
            className="font-mono text-[10px] lg:text-xs px-1 lg:px-1.5 py-0.5 rounded"
            style={{ 
          ...
          <XCircle className="w-3 h-3 lg:w-4 lg:h-4 text-danger" />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-0.5 lg:mt-1">
        <span 
          className="text-[10px] lg:text-xs font-mono uppercase"
          style={{ color: categoryColor }}
        >
          {attack.category.replace('_', ' ')}
        </span>
        <span className="text-[10px] lg:text-xs text-danger font-bold">
          REJECTED
        </span>
      </div>
    </motion.div>
